...
class _InvoiceViewState extends State<_InvoiceView> {
  ...
  Future<bool> _onWillNavigateAway() async {
    ...
    if (!state.isSaved &&
        (state.items.isNotEmpty || state.selectedCustomer != null)) {
      final shouldLeave = await showDialog<bool>(
        context: context,
        builder: (ctx) => AlertDialog(
          backgroundColor: AppColors.surfaceDark,
          title: const Text(
            'فاکتور ذخیره نشده است',
            style: TextStyle(color: AppColors.error),
          ),
          content: const Text(
            'آیا مطمئن هستید که می‌خواهید خارج شوید؟ اطلاعات ثبت شده از دست خواهد رفت.',
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      _ShortcutBadge(
                        label: 'Ctrl+S',
                        description: AppStrings.save,
                      ),
                      const SizedBox(width: 8),
                      _ShortcutBadge(
                        label: 'F2',
                        description: AppStrings.addItem,
                      ),
                    ],
                  ),
                  ...
                  // ── Items Grid ────────────────────────────────────
                  Expanded(child: const InvoiceItemsGrid()),
  ...
  Future<void> _previewInvoice(BuildContext context) async {
    ...
    if (state.items.isEmpty) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('فاکتور خالی است')));
      return;
    }
  ...
  Future<void> _loadTemplate(BuildContext context) async {
    ...
    if (template != null && context.mounted) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text(AppStrings.templateLoaded)));
    }
  }
  ...
  Future<void> _saveAsTemplate(BuildContext context) async {
    ...
    if (name != null && name.isNotEmpty && context.mounted) {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text(AppStrings.templateSaved)));
      }
    }
  }
}
